            ...
            holder.likefollowbutton.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    if (userHasFollowed == true) {
                        ...
                        mFollowingDataMap.put("user_id", like.getUser_id());
                        HashMap<String, Object> mFollowerDataMap = new HashMap<>();
                        mFollowerDataMap.put("user_id", user_id);
                        mDatabase.child("following").child(user_id).child(like.getUser_id()).setValue(mFollowingDataMap);
                        mDatabase.child("follower").child(like.getUser_id()).child(user_id).setValue(mFollowerDataMap);
        ...
        mDatabase.child("users").child(like.getUser_id()).child("user_data").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (dataSnapshot.child("profile_image").getValue().toString() == null) {
                    Glide.with(mContext).load(R.drawable.default_profile_pic).thumbnail(0.1f).into(holder.likeprofileCIV);
                } else {
