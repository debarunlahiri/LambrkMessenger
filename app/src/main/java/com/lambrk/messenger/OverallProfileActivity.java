...
import com.lambrk.messenger.Group.Group;
import com.lambrk.messenger.Group.GroupAdapter;
import com.lambrk.messenger.Settings.SettingsActivity;
import com.lambrk.messenger.Stories.Stories;
import com.lambrk.messenger.Stories.StoriesActivity;
import com.lambrk.messenger.Utils.FollowingActivity;
...
public class OverallProfileActivity extends AppCompatActivity {
        ...
        mDatabase.child("users").child(user_id).child("user_data").child("profile_background_image").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    Glide.with(mContext).load(profile_background_image).apply(RequestOptions.bitmapTransform(new BlurTransformation(8, 2))).into(overallprofilebackgroundIV);
                } else {
                    Glide.with(mContext).load(R.drawable.overallprofilebg).apply(RequestOptions.bitmapTransform(new BlurTransformation(8, 2))).into(overallprofilebackgroundIV);
                }
            }
                            ...
                            if (followingIsOnRequest == true) {
                                ...
                                if (userHasFollowed == true) {
                                    ...
                                    if (private_profile == true) {
                                        ...
                                        HashMap<String, Object> mFollowingDataMap = new HashMap<>();
                                        mFollowingDataMap.put("user_id", searched_user_id);
                                        HashMap<String, Object> mFollowerDataMap = new HashMap<>();
                                        mFollowerDataMap.put("user_id", user_id);
                                        mDatabase.child("following").child(user_id).child(searched_user_id).setValue(mFollowingDataMap);
                                        mDatabase.child("follower").child(searched_user_id).child(user_id).setValue(mFollowerDataMap);
        ...
        mDatabase.child("users").child(searched_user_id).child("user_data").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (dataSnapshot.child("profile_image").getValue().toString() == null) {
                    Glide.with(mContext).load(R.drawable.default_profile_pic).thumbnail(0.1f).into(profileCIV);
                    Glide.with(mContext).load(R.drawable.default_profile_pic).thumbnail(0.1f).into(groupchatprofileCIV);
                } else {
                ...
                if (dataSnapshot.child("bio").equals("")) {
    ...
    private void getFollowersCount() {
        mDatabase.child("follower").child(searched_user_id).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
